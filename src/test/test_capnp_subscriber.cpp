...
static uint32_t read_schema_id(const std::vector<uint8_t>& hdr) {
    ...
    return static_cast<uint32_t>(hdr[21]) | (static_cast<uint32_t>(hdr[22]) << 8) |
           (static_cast<uint32_t>(hdr[23]) << 16) | (static_cast<uint32_t>(hdr[24]) << 24);
}
...
TEST(CapnpSubscriber, PathA_AlignedPayloadPostsRawBytes) {
    ...
    CapnpSubscriber sub(
        0x0001, [&](const uint8_t* h, uint32_t hl, const uint8_t* p, uint32_t pl, bool aligned) {
    ...
    sub.on_message(0x1234, 0x0001, 0x8001, 0x02, 0x00, 0x00, 0x01, payload, 16);
...
TEST(CapnpSubscriber, PathA_MisalignedPayloadCopiesAndPosts) {
    ...
    sub.on_message(0x1234, 0x0001, 0x8001, 0x02, 0x00, 0x00, 0x01, misaligned, 8);
...
TEST(CapnpSubscriber, EmptyPayloadPostsHeaderOnly) {
    CapnpPostCapture capture;
    CapnpSubscriber sub(
        0x0002, [&](const uint8_t* h, uint32_t hl, const uint8_t* p, uint32_t pl, bool aligned) {
            capture.post(h, hl, p, pl, aligned);
        });

    sub.on_message(0x1234, 0x0001, 0x8001, 0x02, 0x00, 0x00, 0x01, nullptr, 0);
...
TEST(CapnpSubscriber, SchemaIdAccessor) {
    CapnpSubscriber sub(0x0042, [](const uint8_t*, uint32_t, const uint8_t*, uint32_t, bool) {});
...
TEST(CapnpSelectiveDecoder, EmptyPayloadReturnsEmpty) {
    CapnpSelectiveDecoder decoder;
    decoder.register_schema(0x0001,
                            [](const uint8_t*, uint32_t) { return std::vector<uint8_t>{0xFF}; });
...
TEST(CapnpSelectiveDecoder, HasSchema) {
    ...
    decoder.register_schema(0x0001,
                            [](const uint8_t*, uint32_t) { return std::vector<uint8_t>{}; });
...
TEST(CapnpSelectiveDecoder, MultipleSchemas) {
    CapnpSelectiveDecoder decoder;
    decoder.register_schema(0x0001,
                            [](const uint8_t*, uint32_t) { return std::vector<uint8_t>{0x01}; });
    decoder.register_schema(0x0002,
                            [](const uint8_t*, uint32_t) { return std::vector<uint8_t>{0x02}; });
